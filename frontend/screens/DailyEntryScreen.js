...
// ── Mini Date Picker (no external lib needed) ──────────────
    ...
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onCancel}>
      <TouchableOpacity style={dp.overlay} activeOpacity={1} onPress={onCancel}>
        <TouchableOpacity style={dp.sheet} activeOpacity={1}>
          ...
          {/* Date list */}
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={dp.scroll} contentContainerStyle={dp.scrollContent}>
            {dates.map((d, i) => {
              ...
              return (
                <TouchableOpacity
                  key={i}
                  style={[dp.dateChip, isSelected && dp.dateChipActive, isWeekend && !isSelected && dp.dateChipWeekend]}
                  onPress={() => setPicked(d)}
                >
                  <Text style={[dp.chipDay, isSelected && dp.chipDayActive]}>{dayName.slice(0,3)}</Text>
                  <Text style={[dp.chipDate, isSelected && dp.chipDateActive]}>{d.getDate()}</Text>
...
const DailyEntryScreen = ({ navigation }) => {
  ...
  // Selected entry date (the night of this date → decision for next day)
  ...
  const preview = (t1 > 0 || t2 > 0)
    ? getIndentDecisionForDate(t1, t2, entryDate)
    : null;
...
const dp = StyleSheet.create({
  ...
  dateChipWeekend:  { borderColor: '#FED7AA', backgroundColor: '#FFF7ED' },
