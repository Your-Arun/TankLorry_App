...
const { getIndentDecision, getSaleRate } = require('../utils/decisionEngine');
...
const createEntry = async (req, res) => {
  try {
    ...
    const d = getIndentDecision(Number(tank1), Number(tank2));

    const entry = await StockEntry.create({
      tank1: Number(tank1), tank2: Number(tank2),
      avgSale: d.tomorrowSaleRate,  // auto-set from day
      totalStock,
...
const getHistory = async (req, res) => {
  try {
    ...
    const [entries, total] = await Promise.all([
      StockEntry.find().sort({ createdAt: -1 }).skip((page-1)*limit).limit(limit),
